 ...
 *
 */
public class ArabianSearchServlet extends HttpServlet {
	...
	private static final Logger LOG = Logger.getLogger(ArabianSearchServlet.class
			.getName());
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		req.getRequestDispatcher("arabian_search.jsp").forward(req, resp);
	}
	...
	private String add(HttpServletRequest req, User currentUser) {
		...
		Document.Builder docBuilder = Document
				.newBuilder()
				.addField(
						Field.newBuilder().setName("content").setText(content))
				.addField(
				...
				.addField(
						Field.newBuilder().setName("published")
								.setDate(new Date()))
				.addField(
						Field.newBuilder().setName("rating").setNumber(rating));
	...
	private String getOnlyField(Document doc, String fieldName,
			String defaultValue) {
	...
	private String search(HttpServletRequest req) {
		...
		try {
			...
			Query query = Query.newBuilder()
					...
					.setOptions(QueryOptions.newBuilder().build()).build(queryStr);
 ...
 */
	 ...
	 * types of actions: ADD, REMOVE and DEFAULT. The DEFAULT is included to
	 * indicate action other than ADD or REMOVE. We do not have a special action
	 * for search, as we always execute search. This way we show documents that
	 * match terms entered in the search box, regardless of the operation.
	 *
	 * @param HTTP
